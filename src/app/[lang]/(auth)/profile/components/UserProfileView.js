...
export default function UserProfileView({
  ...
  const pronouns = [
    ...
    {
      value: "not-specified",
      label: pageContent["body-profile-gender-notespecified"],
    },
  ...
  useEffect(() => {
    if (
      userProfileData?.skillData !== undefined &&
      userProfileData?.userData !== undefined &&
      userProfileData.userData !== ""
    ) {
      setWantedCapacities(
        userProfileData.skillData.filter((item) =>
          userProfileData.userData.skills_wanted.includes(item.code)
        )
      );
      setKnownCapacities(
        userProfileData.skillData.filter((item) =>
          userProfileData.userData.skills_known.includes(item.code)
        )
      );
      setAvailableCapacities(
        userProfileData.skillData.filter((item) =>
          userProfileData.userData.skills_available.includes(item.code)
        )
      );
      setContactCount(
        userProfileData.userData.contact?.filter(
          (item) => item.display_name !== "" || item.value !== ""
        ) ?? []
      );
    }
  }, [userProfileData]);
  ...
  if (userProfileData === undefined) {
    return <UserProfileViewSkeleton darkMode={darkMode} />;
  }
  if (userProfileData.userData === "") {
    return <NoUserProfileView darkMode={darkMode} pageContent={pageContent} />;
  }
      ...
      <div className="w-full sm:w-8/12 mx-auto space-y-28">
          ...
          <div className="space-y-6 sm:space-y-4">
            <div className="flex-none sm:flex sm:space-x-4 space-y-4 sm:space-y-0 justify-center">
              ...
              >
                <Link
                  href={
                    "https://meta.wikimedia.org/wiki/User:" +
            ...
            <div className="space-y-6 sm:space-y-4">
              ...
              <div
                className={
                  ...
                  "w-fit px-4 py-2 mx-auto my-auto rounded-lg"
                }
              >
                {userProfileData.userData.pronoun ? (
                  <h3 className="text-base">
                    {pronouns.map((option) =>
                      option.value === userProfileData.userData.pronoun
                        ? option.label
                        : null
                    )}
                  </h3>
                  ...
                  <h3 className="text-base">
                    {pageContent["body-profile-gender-notespecified"]}
                  </h3>
                )}
              </div>
            </div>
        ...
        {contactCount?.length === 0 && socialMediaCount?.length === 0 ? null : (
          ...
          <section className="space-y-6 sm:space-y-4">
            ...
            {/* Contact */}
            {userProfileData.userData.contact?.map((item, index) =>
              item.display_name === "" || item.value === "" ? null : (
                <ViewTextBox
                  key={index}
                  darkMode={darkMode}
        ...
        <section className="space-y-14 sm:space-y-14">
          {userProfileData.userData.skills_wanted.length === 0 ? null : (
            <div className="space-y-6 sm:space-y-4">
              ...
              <UserProfileViewCapacityBox
                darkMode={darkMode}
                data={wantedCapacities}
                endpoint={"capacity"}
              />
            </div>
          )}
