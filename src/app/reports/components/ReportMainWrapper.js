...
import EditReportForm from "@/app/reports/[id]/edit/components/EditReportForm";
import ForbiddenReportEditView from "@/app/reports/[id]/edit/components/ForbiddenReportEditView";
...
export default function ReportMainWrapper(props) {
    ...
    if (status === "loading") {
        pageComponent = (<LoadingSection darkMode={darkMode} message="THE REPORT DATA FOR EDITING"/>)
    }

    if (status === "authenticated") {
        if (!authorIsUser && reportData?.id) {
            pageComponent = (<ForbiddenReportEditView darkMode={darkMode} pageContent={pageContent}/>)
        } else if (reportData !== undefined) {
            pageComponent = (<EditReportForm darkMode={darkMode} session={{sessionstatus: status, sessionData: data}}
