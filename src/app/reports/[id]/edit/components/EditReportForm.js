...
export default function EditReportForm({darkMode, session, reportData, setReportData, pageContent}) {
    ...
    return (
        <section className={"flex flex-wrap flex-col w-10/12 h-fit mx-auto place-content-start py-32"}>
            ...
            <div className="flex items-start flex-wrap w-full sm:flex-nowrap">
                <div
                    className="flex space-x-4 flex-wrap sm:w-fit w-full sm:flex-nowrap justify-between sm:justify-start mb-6">
                    <SubmitButton updatingData={updatingData} form="report_form">{pageContent["form-report-update-button"]}</SubmitButton>
                    <ButtonRedirectToPage to={"/reports/"+reportData.id}>Cancel </ButtonRedirectToPage>
                </div>
            </div>
        </section>
    )
